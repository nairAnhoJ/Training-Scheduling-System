...
@section('content')
    ...
    <div class="p-5 w-full h-[calc(100%-56px)] bg-gray-200">
        <div class="bg-white shadow-xl rounded-lg p-5 h-full max-h-full overflow-y-auto">
            <form action="{{ route('users.update', ['key' => $key]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    <div class="mb-3 w-full">
                        ...
                        <input type="text" id="first_name" name="first_name" value="{{ $user->first_name }}" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg block max-w-96 w-full p-2.5" required autocomplete="off">
                    </div>
                    <div class="mb-3 w-full">
                        <label for="last_name" class="block text-sm font-semibold text-gray-600">Last Name <span class="text-red-500">*</span></label>
                        <input type="text" id="last_name" name="last_name" value="{{ $user->last_name }}" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg block max-w-96 w-full p-2.5" required autocomplete="off">
                    </div>
                    <div class="mb-3 w-full">
                        <label for="id_number" class="block text-sm font-semibold text-gray-600">ID Number <span class="text-red-500">*</span></label>
                        <input type="text" id="id_number" name="id_number" value="{{ $user->id_number }}" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg block max-w-96 w-full p-2.5" required autocomplete="off">
                    </div>
                    ...
                    <div class="mb-3 {{ $user->id == 1 ? 'opacity-50' : '' }}">
                        ...
                        <select {{ $user->id == 1 ? 'disabled' : '' }} id="department" name="department" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block max-w-96 w-full p-2.5">
                    ...
                    <div class="mb-3 w-full">
                        ...
                        <input type="text" id="email" name="email" value="{{ $user->email }}" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg block max-w-96 w-full p-2.5" required autocomplete="off">
                    </div>

                    <div class="mb-3 {{ $user->id == 1 ? 'opacity-50' : '' }}">
                        <label for="role" class="block text-sm font-semibold text-gray-600">Role <span class="text-red-500">{{ $user->id == 1 ? '' : '*' }}</span></label>
                        <select {{ $user->id == 1 ? 'disabled' : '' }} id="role" name="role" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block max-w-96 w-full p-2.5">
                            ...
                            <option {{ $user->role == 3 ? 'selected' : '' }} value="3">Trainer Head</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-5 flex gap-x-8 max-w-96 w-full">
